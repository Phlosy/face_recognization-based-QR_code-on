...
def Read():
    global Data_Array
    Data_Array = open('data/Authorited.txt').read().splitlines()  # 按行分隔
...
def Read_Decode_Pic(image):
    ...
    for code in decode(image):
        ...
        cv2.polylines(image, [pts_poly], True, (0, 255, 0), 5)  # 画多边形框

        # 显示数据（获取矩形框的左上角作为Text的坐标(左边坐标)，显示数据）
        pts_rect = code.rect
        # print(pts_rect)
        cv2.putText(image, data, (pts_rect[0], pts_rect[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
